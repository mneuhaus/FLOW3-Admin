 ...
 */
class ResourceObjectConverter implements ConverterInterface {
	...
	public function getSupportedTypes() {
		return array('TYPO3\FLOW3\Resource\Resource');
	}
	...
	public function convertFrom($source) {
		if (is_array($source)) {
			...
			if ($source['error'] !== \UPLOAD_ERR_OK) return $this->objectManager->create('TYPO3\FLOW3\Error\Error', \TYPO3\FLOW3\Utility\Files::getUploadErrorMessage($source['error']) , 1264440823);
			...
			if ($resource === FALSE) {
				return $this->objectManager->create('TYPO3\FLOW3\Error\Error', 'The resource manager could not create a resource instance.' , 1264517906);
			} else {
				$this->convertedResources[$source['tmp_name']] = $resource;
				return $resource;
			}
		} else {
			return $this->objectManager->create('TYPO3\FLOW3\Error\Error', 'The source for conversion to a resource object was not an array.' , 1264440811);
		}
	}
