 ...
 */
class PHPCRAdapterTest extends \Testing\BaseTestCase {
    /**
	...
	public function setUp() {
        $this->objectManager = \Admin\Core\Register::get("objectManager");
	}

    public function getAdapter(){
        $a = $this->objectManager->get("Admin\Adapters\PHPCRAdapter");
        $a->init();
        return $a;
    }
    ...
    public function testCreateObjectWithTagModel() {
        $a = $this->getAdapter();
        $r = $a->createObject("Admin\Domain\Model\Tag", array(
            "name" => "UnitTest"
        ));
    ...
    public function testUpdateObjectWithTagModel() {
        $o = \Admin\Core\Register::get("tag");
        $a = $this->getAdapter();
        $id = $a->getId($o);
        
        $r = $a->updateObject("Admin\Domain\Model\Tag", $id, array(
    ...
    public function testDeleteObjectWithTagModel() {
        ...
        $a->deleteObject("Admin\Domain\Model\Tag", $id);

        $this->assertNull($a->getObject("Admin\Domain\Model\Tag", $id));
    }
    ...
    public function testCreateWidgetModel() {
        ...
        $r = $a->createObject("Admin\Domain\Model\Widgets", array(
       ...
       #\Admin\Core\Register::add("cleanup",null,$tagId);
       #\Admin\Core\Register::add("cleanup",null,$a->getId($o));
    }
    ...
    public function testCreateRelationModel() {
        $a = $this->getAdapter();
        $r = $a->createObject("Admin\Domain\Model\Tag", array("name" => "FLOW3"));
        $tagId = $a->getId($r["object"]);
        $r = $a->createObject("Admin\Domain\Model\Tag", array("name" => "Hello"));
        $tagId2 = $a->getId($r["object"]);
        
        $r = $a->createObject("Admin\Domain\Model\Relation", array(
            "tag" 	=> $tagId,
            "tags"	=> array($tagId),
            "inlineWidget" => array(
        ...
        \Admin\Core\Register::set("widget",$o);
    }
    ...
    public function testUpdateRelationModel() {
        $a = $this->getAdapter();
        $relation = \Admin\Core\Register::get("widget");
