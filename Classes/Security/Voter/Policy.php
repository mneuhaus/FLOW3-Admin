<?php
namespace Admin\Security\Voter;
 ...
 */
class Policy extends \TYPO3\FLOW3\Security\Authorization\Voter\Policy {
	...
	public function voteForResource(\TYPO3\FLOW3\Security\Context $securityContext, $resource) {
		...
		foreach ($securityContext->getRoles() as $role) {
			...
			if ($privilege === \TYPO3\FLOW3\Security\Policy\PolicyService::PRIVILEGE_GRANT) $accessGrants++;
			else $accessDenies++;
		}
